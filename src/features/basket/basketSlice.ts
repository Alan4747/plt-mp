...
export const basketSlice = createSlice({
  ...
  reducers: {
    addItemToBasket: (state, action: PayloadAction<ItemType>) => {
      ...
      const itemIndex = state.items.findIndex((b) => b.item.id === newItem.id);
    ...
    deleteAllItems: (state, action: PayloadAction<ItemType>) => {
      const newItem = action.payload;
      const itemIndex = state.items.findIndex((b) => b.item.id === newItem.id);
      if (itemIndex !== -1) {
        state.items.splice(itemIndex, 1);
...
export const { addItemToBasket, reduceItemFromBasket, deleteAllItems } =
  basketSlice.actions;
